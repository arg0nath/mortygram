...
class OnBoardingCubit extends Cubit<OnBoardingState> {
  OnBoardingCubit({required CacheFirstTimer cacheFirstTimer, required CheckFirstTimer checkFirstTimer})
  ...
  Future<void> cacheFirstTimer() async {
    ...
    result.fold((Failure failure) => emit(OnBoardingError(failure.errorMessage)), (_) => emit(const UserCached()));
  }
  ...
  Future<void> checkIfUserFirstTimer() async {
    ...
    result.fold((Failure failure) => emit(const OnBoardingStatus(isFirstTimer: true)), (bool status) => emit(OnBoardingStatus(isFirstTimer: status)));
  }
}
